...
describe("draftFlowFromIntent", () => {
	it("turns intent into a runnable, versioned, identified Markdown flow", () => {
		const draft = draftFlowFromIntent(
			"Review database migrations for rollback risks",
			{
				version: "4.2.0",
				flowId: "flow_test123",
			},
	...
	it("refuses effort without an engine or on an engine with no effort control", () => {
		...
		expect(() =>
			draftFlowFromIntent("Do a thing", { engine: "gemini", effort: "high" }),
		).toThrow("no verified reasoning-effort control");
	});
});
...
describe("suggestFlowSlug", () => {
	it("keeps only the first few meaningful words", () => {
		expect(
			suggestFlowSlug("Review database migrations for rollback risks"),
		).toBe("review-database-migrations");
		expect(
			suggestFlowSlug("Summarize this repository for a new contributor"),
		).toBe("summarize-repository-contributor");
		expect(suggestFlowSlug("Draft release notes from recent commits")).toBe(
			"draft-release-notes",
		);
	});
...
describe("composer engine metadata", () => {
	...
	it("translates effort per engine and hides it where unsupported", () => {
		...
		expect(() => effortFrontmatter("copilot", "high")).toThrow(
	...
	it("rejects effort levels the engine does not support", () => {
		...
		expect(() => effortFrontmatter("claude", "banana")).toThrow(
			"low, medium, high",
		);
		expect(() => effortFrontmatter("codex", "max")).toThrow(
...
describe("docReferenceLine", () => {
	it("classifies commands, URLs, paths, and bare tool names", () => {
		...
		expect(docReferenceLine("https://example.com/docs.md")).toBe(
			"@https://example.com/docs.md",
		);
		expect(docReferenceLine("./docs/api.md")).toBe("@./docs/api.md");
		expect(docReferenceLine("gog")).toBe("!`gog --help`");
		expect(docReferenceLine("@./already-an-import.md")).toBe(
			"@./already-an-import.md",
		);
		expect(docReferenceLine("!git log --oneline -5")).toBe(
			"!`git log --oneline -5`",
		);
	});

	it("refuses a path with spaces instead of turning it into a shell command", () => {
		expect(() => docReferenceLine("./docs/API Guide.md")).toThrow(
			"cannot express",
		);
		expect(() => docReferenceLine("./docs/API Guide.md")).toThrow(
			"prefix with !",
		...
		expect(docReferenceLine("!cat './docs/API Guide.md'")).toBe(
...
describe("slugifyFlowIntent", () => {
	it("normalizes unicode, bounds length, and protects the roster filename", () => {
		...
		expect(slugifyFlowIntent("word ".repeat(30)).length).toBeLessThanOrEqual(
			56,
		);
	});
});
...
describe("applyFlowDraft", () => {
	it("creates the flow, roster, and config additively", () => {
		...
		).toContain("MDFLOW_DRAFT_CASE");
		expect(
			readFileSync(join(directory, "flows", "README.md"), "utf8"),
		).toContain("Open the Flow Workbench: `md`");
		expect(readFileSync(join(directory, ".mdflow.yaml"), "utf8")).toContain(
			"engine: codex",
		);
	});
	...
	it("never overwrites an existing flow or creates support files on conflict", () => {
		...
		const draft = draftFlowFromIntent("Review changes", {
			flowId: "flow_review",
			version: "4.1.0",
		});
	...
	it("preserves user roster text while updating the managed block and config", () => {
		mkdirSync(join(directory, "flows"), { recursive: true });
		writeFileSync(join(directory, "flows", "README.md"), "MY ROSTER\n");
		writeFileSync(join(directory, ".mdflow.json"), '{"engine":"claude"}\n');
		const draft = draftFlowFromIntent("Draft release notes", {
			flowId: "flow_release",
			version: "4.1.0",
		});
		...
		expect(result.status).toBe("created");
		const roster = readFileSync(join(directory, "flows", "README.md"), "utf8");
		expect(roster).toStartWith("MY ROSTER\n");
		expect(roster).toContain("<!-- mdflow:managed:start contract=1 -->");
		expect(roster).toContain("draft-release-notes.md");
		expect(readFileSync(join(directory, ".mdflow.json"), "utf8")).toBe(
			'{"engine":"claude"}\n',
		);
		expect(existsSync(join(directory, ".mdflow.yaml"))).toBe(false);
		expect(result.skipped).toEqual([join(directory, ".mdflow.json")]);
	});
});
...
function run(
	...
	return {
		...
		flow: {
			id: flowId,
			path: "/repo/flows/one.md",
			relativePath: "flows/one.md",
		},
...
describe("summarizeFlowLifecycle", () => {
	it("prioritizes a verified proposal over open evidence", () => {
		...
		expect(summary.evolution).toMatchObject({
			latestStatus: "verified_improvement",
			reviewable: true,
		});
		expect(summary.headline).toBe(
			"1 open feedback · 1 targeted · verified improvement ready",
		);
