...
describe("parseCreateArgs", () => {
	...
	it("rejects missing values and unknown locations", () => {
		expect(() => parseCreateArgs(["hello", "--engine"])).toThrow(
...
describe("runCreate", () => {
	it("creates a canonical project flow, roster, and config from intent", async () => {
		...
		const result = await runCreate(
			["Review", "staged", "changes", "for", "correctness"],
			{
				cwd: directory,
		...
		expect(messages.join("\n")).toContain(
			"Run it directly: md review-staged-changes",
		);
	...
	it("uses exactly one intent prompt when invoked bare", async () => {
		...
		expect(
			existsSync(
				join(directory, "flows", "summarize-repository-contributor.md"),
			),
		).toBe(true);
	});
	...
	it("warns when unknown flags are retained as frontmatter", async () => {
		...
		const result = await runCreate(
			["Test flow", "--dry-run", "--scope", "global"],
			{
				cwd: directory,
				log: () => {},
	...
	it("previews the exact draft and target without writing", async () => {
		const messages: string[] = [];
		const result = await runCreate(
			["Draft release notes", "--engine", "codex", "--dry-run"],
			{
				cwd: directory,
				log: (message) => messages.push(message),
			},
		...
		expect(output).toContain(
			"Would create in THIS PROJECT → ./flows/draft-release-notes.md",
		);
		expect(output).toContain("Effect: FREE — no files written");
		expect(output).toContain("engine: codex");
		expect(output).toContain(
			join(directory, "flows", "draft-release-notes.md"),
		);
	});
	...
	it("creates a genuinely global flow and runs the availability check", async () => {
		...
		expect(result.status).toBe("created");
		if (result.status !== "created") throw new Error("expected created result");
		expect(result.flowPath).toBe(
			join(home, ".mdflow", "flows", "review-globally.md"),
		);
		expect(receipts).toEqual([
			{
				flowPath: result.flowPath,
				invocation: "md review-globally",
				slug: "review-globally",
				userAgentsDir: join(home, ".mdflow", "flows"),
			},
		]);
		const output = messages.join("\n");
		expect(output).toContain(
			"Creating GLOBALLY → ~/.mdflow/flows/review-globally.md (available from any directory as md review-globally)",
		);
		expect(output).toContain(
			"Created GLOBALLY → ~/.mdflow/flows/review-globally.md (available from any directory as md review-globally)",
		);
		expect(output).toContain("Run from any directory: md review-globally");
		expect(output).toContain(
			"Exact global path (bypasses a same-named project flow): md ~/.mdflow/flows/review-globally.md",
		);
	});
	...
	it("resolves a global flow by name from an unrelated directory", async () => {
		...
		expect(created.stdout).toContain(
			"Run from any directory: md global-availability-probe",
		);
		expect(
			existsSync(
				join(home, ".mdflow", "flows", "global-availability-probe.md"),
			),
		).toBe(true);

		const invoked = await spawnMd(["global-availability-probe", "--_dry-run"], {
			cwd: elsewhere,
			env,
		});
	...
	it("never overwrites an existing flow", async () => {
		const first = await runCreate(["Review changes"], {
			cwd: directory,
			log: () => {},
		});
	...
	it("keeps custom-directory creation as a safe, create-only legacy escape hatch", async () => {
		const result = await runCreate(
			["Explain API changes", "--dir", "legacy agents"],
			{
				cwd: directory,
				log: () => {},
			},
		...
		if (result.status !== "created") throw new Error("expected created result");
		expect(result.flowPath).toBe(
			join(directory, "legacy agents", "explain-api-changes.md"),
		);
		expect(existsSync(result.flowPath)).toBe(true);
		expect(existsSync(join(directory, "legacy agents", "README.md"))).toBe(
			false,
		);
		expect(existsSync(join(directory, ".mdflow.yaml"))).toBe(false);
	});
	...
	it("creates in the current directory with scope-specific invocation guidance", async () => {
		...
		const output = messages.join("\n");
		expect(output).toContain(
			"Creating in CURRENT DIRECTORY → ./local-helper.md",
		);
		expect(output).toContain(
			"Created in CURRENT DIRECTORY → ./local-helper.md",
		);
...
describe("create scope display never lies about escaping paths", () => {
	it("falls back to the absolute path when the custom target escapes the base directory", () => {
		const home = join(directory, "home");
		const escapingFlowPath = resolve(
			directory,
			"..",
			"shared-out",
			"review.md",
